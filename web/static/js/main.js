    ...
    return new Promise((resolve) => {
        fetch('/api/status', {
            ...
            .then(data => {
                ...
                // Dispatch data-refreshed event for advanced mode
                document.dispatchEvent(new CustomEvent('data-refreshed', { detail: data }));

                // Update last refreshed timestamp
                const lastRefreshed = document.getElementById('last-refreshed');
                if (lastRefreshed) {
                    const span = lastRefreshed.querySelector('span');
                    if (span) {
                        span.textContent = `Last refreshed: ${new Date().toLocaleTimeString()}`;
                    }
                }
    ...
    return new Promise((resolve) => {
        fetch('/api/status', {
            ...
            .then(data => {
                ...
                const loginWarningNotice = document.getElementById('login-warning-notice');
                  if (data.username) {
                    // User is logged in
                    if (userStatus) {
                    ...
                    // Hide login warning notice when user is logged in
                    if (loginWarningNotice) {
                        loginWarningNotice.classList.add('hidden');
                    }
                } else {
                    ...
                    // Show login warning notice when user is not logged in
                    if (loginWarningNotice) {
                        loginWarningNotice.classList.remove('hidden');
                    }
...
function updateSettingsUI(data) {
    ...
    // Update Connection Quality select (hidden in web UI)
    ...
    // Update Language select (hidden in web UI)
    ...
    // Update checkboxes (some are hidden in web UI)
    ...
    if (darkModeCheckbox && data.hasOwnProperty('dark_mode')) {
        darkModeCheckbox.checked = data.dark_mode;
    }

    const unlinkedCampaignsCheckbox = document.getElementById('unlinked-campaigns-checkbox');
    if (unlinkedCampaignsCheckbox && data.hasOwnProperty('unlinked_campaigns')) {
        unlinkedCampaignsCheckbox.checked = data.unlinked_campaigns;
    }
...
function saveSettings(reloadAfterSave = false) {
    ...
    const settings = {
        ...
        // Note: Language, connection_quality, autostart_tray, tray_notifications are desktop-specific
        // and not included here since they're hidden in the web UI
        dark_mode: document.getElementById('dark-mode-checkbox').checked,
        unlinked_campaigns: document.getElementById('unlinked-campaigns-checkbox').checked,
