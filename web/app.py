...
from web.auth import is_setup_needed, create_user, validate_credentials, login_user, logout_user, auth_required, login_required, get_session_secret
...
def initialize(loop, twitch_instance):
    ...
    # Ensure session secret is generated now that webui is enabled
    get_session_secret()
...
app.config['PERMANENT_SESSION_LIFETIME'] = 7 * 24 * 60 * 60  # 7 days in seconds
    ...
    try:
        ...
        if hasattr(twitch, '_last_drop_update') and twitch._last_drop_update:
            ...
            if websocket_drop:
                ...
                time_remaining = format_time_remaining(websocket_drop.remaining_minutes)
        ...
        if current_drop is None:
            ...
            if active_drop:
                ...
                time_remaining = format_time_remaining(active_drop.remaining_minutes)
...
def settings(username=None):
    ...
    try:
        ...
        settings_data = {
            ...
            'dark_mode': twitch.settings.dark_mode if hasattr(twitch, 'settings') else False,
            'unlinked_campaigns': twitch.settings.unlinked_campaigns if hasattr(twitch, 'settings') else False,
...
def update_settings(username=None):
    ...
    try:
        ...
        if 'dark_mode' in data:
            twitch.settings.dark_mode = bool(data['dark_mode'])

        if 'unlinked_campaigns' in data:
            twitch.settings.unlinked_campaigns = bool(data['unlinked_campaigns'])
    ...
    try:
        ...
        if active_drop:            # Get image URL from the first benefit if available
            ...
            result = {
                ...
                'progress_percentage': round((active_drop.current_minutes / active_drop.required_minutes) * 100) if active_drop.required_minutes > 0 else 0,
...
def auth_validate():
    """Validate if user is authenticated via session"""
    from web.auth import is_logged_in, get_current_user
